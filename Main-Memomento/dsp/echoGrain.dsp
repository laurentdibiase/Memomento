...
echoFx = +~ @(echoDelay)*(fdbck);

echoDelay = hslider("Echo Delay[unit:s]", 1, 0.01, 4, 0.001):min(1):max(0.01)*(44100):int;
fdbck = hslider("Echo Intensity (Feedback)[style:knob][acc:0 1 -10 0 10]", 0.05, 0.001, 0.99, 0.001):si.smooth(0.999);
...
decal = 1 - hslider("decal[BELA: ANALOG_2]",0.1,0,1,0.001);			// Read position compared to table write position

speed = hslider("speed[BELA: ANALOG_3]", 1.75, 0.125, 4, 0.001);

feedback = hslider("feedback[BELA: ANALOG_4]",1.25,0,2,0.001);	
...
grain = _,_ <: ((+(_,_) :processus) ~ (*(feedback))),((+(_,_) :processus) ~ (*(feedback)));



process = echoFx,echoFx : grain ;