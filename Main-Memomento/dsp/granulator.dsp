process = vgroup("granulator",environment{declare name "granulator";
...
gain = 1;
freq = hslider("[1]Grain Size[osc:grnlt/1][acc:2 0 -10 0 10]", 200,5,2205,1);
...
speed = hslider ("[2]Speed[unit:Hz][osc:grnlt/2][acc:0 1 -10 0 10]", 10,1,20,0.0001):fi.lowpass(1,1); 
ratio_env = 0.5;
fade = (0.5); // min > 0 pour eviter division par 0
proba = hslider ("[3]Probability[unit:%][osc:grnlt/3][acc:1 0 -10 0 10]", 70,50,100,1) * (0.01):fi.lowpass(1,1);
