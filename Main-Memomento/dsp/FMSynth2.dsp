...
midifreq = nentry("freq[unit:Hz]", 440, 1, 20000, 0.01);
midigain = nentry("gain", 1, 0, 1, 0.01);

// modwheel:
feedb = (gFreq-1) * (hslider("feedb[midi:ctrl 1]", 0, 0, 1, 0.001) : si.smoo);
modFreqRatio = hslider("ratio",2,0,20,0.01) : si.smoo;
...
fldw = hslider("dryWetFlang",0.5,0,1,0.001);
flanger = efx
	with {
...
reverb = _,_ <: (*(g)*fixedgain, *(g)*fixedgain :
	...
	*(1-g), *(1-g) :> _,_
    with {
        ...
        combfeed = vslider("RoomSize", 0.7, 0, 1, 0.025)*scaleroom*origSR/ma.SR + offsetroom;
        spatSpread = vslider("Stereo[midi:ctrl 90]",0.6,0,1,0.01)*46*ma.SR/origSR;
        g = vslider("dryWetReverb", 0.4, 0, 1, 0.001);
        // (g = Dry/Wet)
    };
