...
/* Quadri amplifier stage with bass, treble, gain and balance controls and a
   dB meter. */

declare name "amp";
declare description "quadri amplifier stage";
...
bass_gain	= nentry("[1] bass [midi:ctrl 23] [unit:dB]", 0, -20, 20, 0.1);
treble_gain	= nentry("[2] treble [midi:ctrl 24] [unit:dB]", 0, -20, 20, 0.1);

/* Gain and balance controls. */

gain		= smooth(ba.db2linear(g)) with {
  g = nentry("[3] gain [midi:ctrl 21] [unit:dB]", 0, -60, 12, 0.1);
};
bal		= smooth(b) with {
  b = hslider("balance [midi:ctrl 22]", 0, -1, 1, 0.001);
};
...
fst_meter(x)	= attach(x, env(x) : hbargraph("[4]one [midi:ctrl 18] [osc:/mtr1 -60 10] [unit:dB]", -60, 10));
scd_meter(x)	= attach(x, env(x) : hbargraph("[5]two [midi:ctrl 19] [osc:/mtr2 -60 10] [unit:dB]", -60, 10));
thd_meter(x)	= attach(x, env(x) : hbargraph("[6]three [midi:ctrl 19] [osc:/mtr3 -60 10] [unit:dB]", -60, 10));
frt_meter(x)	= attach(x, env(x) : hbargraph("[7]four [midi:ctrl 19] [osc:/mtr4 -60 10] [unit:dB]", -60, 10));

/* The main program. */

process		= hgroup("[1]", (tone, tone, tone, tone) : (_*gain, _*gain, _*gain, _*gain))
		//: vgroup("[2]", balance)
		: vgroup("[3]", (fst_meter, scd_meter, thd_meter, frt_meter));
